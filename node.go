...
func ParseNodeMetrics(input []byte) map[string]*NodeMetrics {
	...
	for _, line := range linesUniq {
		...
		// Vérifiez que nous avons au moins 6 champs
		if len(node) < 6 {
		...
		// Vérifiez que nous avons au moins 4 champs CPU
		if len(cpuInfo) < 4 {
			log.Printf("Skipping malformed CPU info: %s", node[3])
			continue
		}
		...
		if nodeGpu != "(null)" {
			...
			// Vérifiez que nous avons au moins 3 champs GPU
			if len(availableGRES) < 3 || len(usedGRES) < 3 {
				log.Printf("Skipping malformed GPU info: %s", node[5])
				continue
			}
